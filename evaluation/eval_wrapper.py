                    ...
                    for k in range(out_j.shape[0]):
                        if out_j[k, i] > 0:
                            fp.write(
                                ...
                                '%d %d ' % (int(out_j[k, i] * col_sample_w * 1920 / 800) - 1, int(culane_row_anchor[(18-1-k)]/288*1080) - 1 ))
                    fp.write('\n')
...
def call_culane_eval(data_dir, exp_name,output_path):
    ...
    im_h=1080
    frame=1
    list0 = os.path.join(data_dir,'list/test.txt')
    ...
    out0 = os.path.join(output_path,'txt','result.txt')
